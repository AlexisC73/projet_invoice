import { invoiceBuilder, productBuilder } from '../../tests/invoiceBuilder'
import {
  UserFixture,
  createUserFixture,
...
describe('Post Invoice', () => {
  ...
  test('should post new Invoice', async () => {
    ...
    await invoiceFixture.whenUserPostInvoice(
      invoiceBuilder().getProps(),
      userFixture.getToken()
    )

    invoiceFixture.thenInvoiceShouldBe(
      invoiceBuilder().withOwner('user-id').build()
    )
  })
  ...
  test('should post new Invoice with draft status', async () => {
    ...
    invoiceFixture.thenInvoiceShouldBe(
      invoiceBuilder().withStatus('draft').withOwner('user-id').build()
    )
  })
  ...
  describe('Rule: default currency must be USD', () => {
    test('should post a new invoice with USD when no currency provided', async () => {
      ...
      await invoiceFixture.whenUserPostInvoice(
        invoiceBuilder().getPropsWithoutCurrency(),
        userFixture.getToken()
      )

      invoiceFixture.thenInvoiceShouldBe(
        invoiceBuilder().withOwner('user-id').withCurrency('USD').build()
      )
    })
    test('should use the EUR currency when provided', async () => {
      ...
      await invoiceFixture.whenUserPostInvoice(
        invoiceBuilder().withCurrency('EUR').getProps(),
        userFixture.getToken()
      )

      invoiceFixture.thenInvoiceShouldBe(
        invoiceBuilder().withOwner('user-id').withCurrency('EUR').build()
      )
    })
  })
  ...
  describe('Rule: new invoice must have pending status', () => {
    test('should post a new Invoice with pending status', async () => {
      ...
      await invoiceFixture.whenUserPostInvoice(
        invoiceBuilder().withStatus('paid').getProps(),
        userFixture.getToken()
      )

      invoiceFixture.thenInvoiceShouldBe(
        invoiceBuilder().withOwner('user-id').withStatus('pending').build()
      )
    })
  })
})
